"""ASR: pyannote → WhisperX → LLM → speech_*.wav/txt (PRD шаг 3).

Локальная отладка: python asr.py
Пайплайн вызывает init_asr_models() один раз, затем run_segment_pipeline() на каждый segment.wav.
"""
import ast
import gc
import json
...
# Только для `python asr.py` — не используется из pipeline/
...
# Один набор моделей на все первичные сегменты (экономия загрузки)
...
def init_asr_models(source_language, hf_token=None):
    """Загрузить pyannote + WhisperX + align (пропуск, если язык уже в памяти)."""
...
def run_segment_pipeline(
    audio_path, output_audio_dir, output_text_dir, source_language,
    hf_token=None, clear_outputs=True, reuse_asr=True,
):
    """Один segment.wav (~40–90 с) → output_*_segments/."""
    token = hf_token or get_hf_token()
    ...
    tr = whisperx.align(
        tr["segments"], _asr["align_model"], _asr["align_meta"], audio, DEVICE,
    ...
    for seg in segments:
        ...
        n += 1
        sf.write(
            out_a / f"speech_{n:03d}_{seg['speaker']}_{seg['start']:.2f}-{seg['end']:.2f}s.wav",
            chunk, 16000,
        )
...
def run_test():
    ...
    try:
        segs = run_segment_pipeline(
            WAV_PATH, OUTPUT_AUDIO_DIR, OUTPUT_TEXT_DIR, SOURCE_LANG, reuse_asr=False,
        )
    finally:
        unload_asr_models()
