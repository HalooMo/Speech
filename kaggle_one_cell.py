...
def patch_runtime(repo: Path, code_src: Path) -> None:
    ...
    for name in ("main.py", "test.py", "llm.py", "prompt.py", "get_param.py", "dubbing.py", "fit_audio.py"):
        src = code_src / name
        if src.is_file():
            shutil.copy2(src, repo / name)
            _log(f"  {name} из dataset")
    ...
    for old in (
        ...
        t = t.replace(old, "", 1)
    if "from get_param import get_emotion, get_sex" not in t:
        t = t.replace(
    ...
    if "from dubbing import dub_from_voice_param" not in t:
        t = t.replace(
            "def dub_segments(\n    second_seg_dir: Path,\n    target_lang: str,\n) -> Path:\n",
            "def dub_segments(\n    second_seg_dir: Path,\n    target_lang: str,\n) -> Path:\n"
